...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "frequency",
            sa.Enum(
                "manual", "daily", "weekly", "biweekly", "monthly", name="frequency"
            ),
            nullable=False,
        ),
        sa.Column(
        ...
        sa.Column(
            "source_type",
            sa.Enum("document", "waf", name="source_type"),
            nullable=False,
        ),
        sa.Column(
    ...
    with op.batch_alter_table("harvest_source", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_harvest_source_frequency"), ["frequency"], unique=False
        )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("in_progress", "complete", "new", "error", name="job_status"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "action",
            sa.Enum("create", "update", "delete", name="record_action"),
            nullable=True,
        ),
        sa.Column(
            "status", sa.Enum("error", "success", name="record_status"), nullable=True
        ),
        sa.Column("id", sa.String(length=36), nullable=False),
        sa.ForeignKeyConstraint(
            ["harvest_job_id"],
    ...
    with op.batch_alter_table("harvest_record", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_harvest_record_action"), ["action"], unique=False
        )
        batch_op.create_index(
        ...
        batch_op.create_index(
            batch_op.f("ix_harvest_record_date_finished"),
            ["date_finished"],
            unique=False,
        )
        batch_op.create_index(
            batch_op.f("ix_harvest_record_status"), ["status"], unique=False
        )
