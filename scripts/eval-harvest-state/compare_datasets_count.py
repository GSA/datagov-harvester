...
saves the results as JSON, compares the dataset counts for each organization between the two catalogs,
and outputs a comparison CSV report.
...
Usage:
    python compare_datasets_count.py [--catalog-url <URL>] [--catalog-next-url <URL>] [--output-csv <FILE>]
...
def compare_orgs(catalog_orgs, catalog_next_orgs, output_csv_path):
    ...
    with open(output_csv_path, "w", newline='', encoding="utf-8") as csvfile:
        ...
        writer.writerow(['Name', 'Organization', 'Catalog Dataset Count', 'Catalog-Next Dataset Count'])
        writer.writerows(rows)

    print(f"\n Comparison report saved to: {output_csv_path} (Total: {len(rows)} rows)")

def main():
    parser = argparse.ArgumentParser(description="Compare dataset counts from two catalogs.")
    parser.add_argument("--catalog-url", default=DEFAULT_CATALOG_URL, help="URL of the current catalog")
    parser.add_argument("--catalog-next-url", default=DEFAULT_CATALOG_NEXT_URL, help="URL of the next catalog")
    parser.add_argument("--output-csv", default=DEFAULT_OUTPUT_CSV, help="Output CSV file for comparison")

    args = parser.parse_args()

    if len(sys.argv) == 1:
        print("\n Using default values (may be overridden them using command-line arguments): ")
    ...
    print(f"\n Fetching organizations with datasets count from current catalog: {args.catalog_url} ...")
    ...
    print(f"\n Fetching organizations with datasets count from next catalog: {args.catalog_next_url} ...")
    catalog_next_orgs = fetch_all_organizations(args.catalog_next_url)
    print(f"   Total organizations fetched from next catalog: {len(catalog_next_orgs)}")
