...
from .decorators import count, count_wrapper, paginate
from .models import (
    Dataset,
    DatasetViewCount,
    HarvestJob,
...
class HarvesterDBInterface:
    ...
    def __init__(self, session=None, *args, **kwargs):
        # Default to the flask-sqlalchemy session so `HarvesterDBInterface()`
        # works inside an app context; the harvest runner passes its own
        # scoped_session explicitly.
        self.db = session if session is not None else db.session
    ...
    def update_dataset_slug(
        ...
        try:
            # Imported here on purpose, not at module scope: harvester.opensearch
            # pulls in harvester/__init__.py, which constructs a
            # HarvesterDBInterface and would import this module back while it is
            # still initializing. Moving this to the top raises ImportError.
