...
def test_publisher_normalized_field_lowercases_values(opensearch_writer):
    """The publisher.normalized subfield must match case-insensitively.

    Salvaged from the upstream reader tests: it exercises the mapping via the
    raw client, so it survives OpenSearchReader being trimmed to scan_index.
    """
    ...
    try:
        ...
        response = opensearch_writer.client.search(
            index=opensearch_writer.INDEX_NAME,
            body={
                "query": {
