...
class TestHarvestJobRestart:
    ...
    def test_harvest_restart_job(
        ...
        job_id = harvest_job.id
        harvest_job_starter(job_id, "restart")
        ...
        assert harvest_job.records[0].ckan_id == '1234'
        assert harvest_job.records[0].status == "success"
        assert harvest_job.records[1].ckan_id == '5678'
        assert harvest_job.records[1].status == "success"

        records_from_db = interface.get_latest_harvest_records_by_source(
            source_data_dcatus_same_title["id"]
        )

        assert len(records_from_db) == 2
