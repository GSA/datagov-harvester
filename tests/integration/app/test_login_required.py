...
def test_harvest_edit__logged_in(client, interface_no_jobs, source_data_dcatus):
    res = client.get(f"/harvest_source/config/edit/{source_data_dcatus['id']}")
    assert res.status_code == 200


# Logged out user cannot see protected page
def test_harvest_edit__logged_out(client, interface_no_jobs, source_data_dcatus):
    res = client.get(f"/harvest_source/config/edit/{source_data_dcatus['id']}")
...
@force_login(email="test@data.gov")
def test_harvest_edit_bad_source_url(client, interface_no_jobs):
...
def test_org_edit_buttons__logged_in(
    client, interface_with_multiple_jobs, source_data_dcatus, organization_data
):
...
def test_org_edit_buttons__logged_out(client, interface_no_jobs, organization_data):
    res = client.get(f"/organization/{organization_data['id']}")
    button_string_text = '<div class="config-actions">'
    org_edit_text = f'<a href="/organization/config/edit/{organization_data["id"]}"'
    org_delete_text = (
...
def test_harvest_data_edit_buttons__logged_in(
    client, interface_no_jobs, source_data_dcatus
):
