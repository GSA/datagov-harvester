...
class TestDatasetUnauthed:
    ...
    def test_dataset_actions_hidden(self, upage):
        action_section = upage.locator(
            ".config-actions.dataset-config-actions.margin-top-3"
        )
...
class TestDatasetAuthed:
    ...
    def test_dataset_actions_visible(self, apage):
        ...
        """
        action_section = apage.locator(
            ".config-actions.dataset-config-actions.margin-top-3"
        )
        expect(action_section).to_be_visible()
        actions = apage.locator(
            ".config-actions.dataset-config-actions.margin-top-3 button"
        )
