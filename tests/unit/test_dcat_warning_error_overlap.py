...
# here, so every test below diffs against this baseline instead of assuming a
# record starts with zero warnings.
#
# Baseline warnings are deliberately *not* precomputed into a module-level
# constant here (unlike VALIDATOR above). Several mutations below (e.g.
# `_unresolvable_spatial_value`) produce an arbitrary string spatial value
# that exercises `translate_spatial`'s DB-backed named-place lookup fallback,
...
# Computing warnings at import time (module collection, before any
# test/fixture runs) would instead reach the real module-level
# `harvester.db_interface` and leak an uncommitted transaction. So
# `_new_warning_types` recomputes the baseline fresh on every call, inside
# the test.
